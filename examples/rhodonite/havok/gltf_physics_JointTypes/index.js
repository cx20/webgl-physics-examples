...
const RESET_Y_THRESHOLD_TOP = 60;
...
const physicsNodes = [];   // { entity, bodyId, isDynamic, initialPosition, initialRotation }
const dynamicNodes = [];
const debugEntities = [];  // wireframe collider indicators
const nodeIndexToBodyId = new Map();  // glTF node index -> Havok bodyId (for joint connections)
const parentOf = new Map();           // child node index -> parent node index
...
function createImplicitShape(shapeDef, scale) {
  ...
  if (shapeDef.type === 'box' && shapeDef.box) {
    ...
    return { shapeId: res[1], size, shapeType: 'box' };
  }
  ...
  if (shapeDef.type === 'sphere' && shapeDef.sphere) {
    ...
    return { shapeId: res[1], size: [r * 2, r * 2, r * 2], shapeType: 'sphere' };
  }
  ...
  if (shapeDef.type === 'capsule' && shapeDef.capsule) {
    ...
    const res = HK.HP_Shape_CreateCapsule([0, -shaftH, 0], [0, shaftH, 0], r);
    checkResult(res[0], 'HP_Shape_CreateCapsule');
    return { shapeId: res[1], size: [r * 2, shaftH * 2 + r * 2, r * 2], shapeType: 'capsule' };
  }
  ...
  if (shapeDef.type === 'cylinder' && shapeDef.cylinder) {
    ...
    if (typeof HK.HP_Shape_CreateCylinder === 'function') {
      ...
      checkResult(res[0], 'HP_Shape_CreateBox (cylinder fallback)');
      shapeId = res[1];
    }
    return { shapeId, size: [r * 2, hh * 2, r * 2], shapeType: 'cylinder' };
  }
...
function createBoxShape(size) {
  ...
  const res = HK.HP_Shape_CreateBox([0, 0, 0], IDENTITY_QUATERNION, safe);
  checkResult(res[0], 'HP_Shape_CreateBox (aabb)');
  return { shapeId: res[1], size: safe, shapeType: 'box' };
}

// Convex hull / triangle mesh collider from glTF mesh geometry (positions/indices in object
// space), scaled by the node's world scale. Returns the shape plus the scaled positions/indices
// (reused for the debug wireframe) and the local AABB.
function createMeshShape(geometry, scale, isConvex, isDynamic) {
  ...
  let indices = geometry.indices.slice();
  // Triangle meshes are one-sided; add reversed winding for static colliders so both faces collide.
  if (!isConvex && !isDynamic) {
    const base = indices.length;
    for (let i = 0; i + 2 < base; i += 3) indices.push(indices[i], indices[i + 2], indices[i + 1]);
  }

  let shapeId = null;
  if (typeof HK._malloc === 'function' && HK.HEAPU8) {
    const posOffset = HK._malloc(positions.length * 4);
    new Float32Array(HK.HEAPU8.buffer, posOffset, positions.length).set(positions);
    try {
      if (isConvex) {
        const r = HK.HP_Shape_CreateConvexHull(posOffset, numVertices);
        checkResult(r[0], 'HP_Shape_CreateConvexHull');
        shapeId = r[1];
      } else {
        const idx = new Uint32Array(indices);
        const triOffset = HK._malloc(idx.length * 4);
        new Int32Array(HK.HEAPU8.buffer, triOffset, idx.length).set(new Int32Array(idx.buffer));
        try {
          const r = HK.HP_Shape_CreateMesh(posOffset, numVertices, triOffset, idx.length / 3);
          checkResult(r[0], 'HP_Shape_CreateMesh');
          shapeId = r[1];
        } finally {
          HK._free(triOffset);
        }
  ...
  if (isDynamic) {
    ...
    if (mp && mp[1]) { /* mass set on the body via createBody */ }
  }

  return { shapeId, size, shapeType: 'mesh', volume, positions, scaledIndices: geometry.indices };
}

// KHR_physics_rigid_bodies motion: override the shape-derived mass properties with the
// glTF values. Spec: mass=0 means infinite mass; an inertiaDiagonal component of 0 means
// that axis is locked. Havok handles those zeros directly. The mass-properties tuple from
// HP_Body_GetMassProperties is [centerOfMass(vec3), mass(scalar), inertiaDiagonal(vec3),
// inertiaOrientation(vec4)] in some order, so probe by slot shape rather than fixed index.
function applyMotionMassProperties(bodyId, motionDef) {
  if (!motionDef || typeof HK.HP_Body_GetMassProperties !== 'function') return;
  ...
  const mpResult = HK.HP_Body_GetMassProperties(bodyId);
  checkResult(mpResult[0], 'HP_Body_GetMassProperties');
  const mp = mpResult[1];
...
// motionDef present => dynamic/kinematic body: build mass from the shape, apply the glTF motion
// overrides (mass / inertia / center of mass / gravityFactor / initial velocities). motionDef
// null => static body (no mass).
function createBody(shapeId, motionType, position, rotation, motionDef) {
  ...
  if (motionDef) {
    const mRes = HK.HP_Shape_BuildMassProperties(shapeId);
    checkResult(mRes[0], 'HP_Shape_BuildMassProperties');
    HK.HP_Body_SetMassProperties(bodyId, mRes[1]);
...
// --- Joints / constraints (KHR_physics_rigid_bodies physicsJoints) ---
function quatInvert(q) { return [-q[0], -q[1], -q[2], q[3]]; } // unit quaternion conjugate
function quatMulVec3(q, v) {
...
function getHKAxisMode(mode) {
  const m = HK.ConstraintAxisLimitMode;
  if (mode === 'FREE') return m ? (m.FREE ?? 0) : 0;
  if (mode === 'LIMITED') return m ? (m.LIMITED ?? 1) : 1;
  return m ? (m.LOCKED ?? 2) : 2;
}
function getHKMotorType(type) {
  const t = HK.ConstraintMotorType;
  if (type === 'VELOCITY') return t ? (t.VELOCITY ?? 1) : 1;
  if (type === 'POSITION') return t ? (t.POSITION ?? 2) : 2;
  return 0;
}

// Resolve a Havok ConstraintAxis enum value (LINEAR_X..ANGULAR_Z) from the runtime enum, with a
// numeric fallback. Using the real enum avoids guessing the axis ordering.
...
// Translate a glTF joint definition (limits + drives) into Havok per-axis constraint settings.
function configureConstraintAxes(constraintId, jointDef) {
  ...
  for (const axis of allHKAxes()) HK.HP_Constraint_SetAxisMode(constraintId, axis, FREE);
  ...
  for (const limit of (jointDef.limits || [])) {
    ...
    for (const axis of axisIndices) {
      if (min === 0 && max === 0) {
        ...
        if (typeof HK.HP_Constraint_SetAxisMinLimit === 'function') HK.HP_Constraint_SetAxisMinLimit(constraintId, axis, min);
        if (typeof HK.HP_Constraint_SetAxisMaxLimit === 'function') HK.HP_Constraint_SetAxisMaxLimit(constraintId, axis, max);
      }
      if (limit.stiffness !== undefined && typeof HK.HP_Constraint_SetAxisStiffness === 'function') HK.HP_Constraint_SetAxisStiffness(constraintId, axis, limit.stiffness);
      if (limit.damping !== undefined && typeof HK.HP_Constraint_SetAxisDamping === 'function') HK.HP_Constraint_SetAxisDamping(constraintId, axis, limit.damping);
    }
  }
  ...
  for (const drive of (jointDef.drives || [])) {
    ...
    let motorType;
    if (hasSpring && hasPosTarget) motorType = getHKMotorType('POSITION');
    else if (hasVelTarget || (drive.damping !== undefined && drive.damping > 0)) motorType = getHKMotorType('VELOCITY');
    else continue;

    HK.HP_Constraint_SetAxisMotorType(constraintId, axis, motorType);
    const target = motorType === getHKMotorType('POSITION') ? (drive.positionTarget ?? 0) : (drive.velocityTarget ?? 0);
    if (typeof HK.HP_Constraint_SetAxisMotorTarget === 'function') HK.HP_Constraint_SetAxisMotorTarget(constraintId, axis, target);
    if (hasSpring && typeof HK.HP_Constraint_SetAxisStiffness === 'function') HK.HP_Constraint_SetAxisStiffness(constraintId, axis, drive.stiffness);
    if (drive.damping !== undefined && typeof HK.HP_Constraint_SetAxisDamping === 'function') HK.HP_Constraint_SetAxisDamping(constraintId, axis, drive.damping);
    if (typeof HK.HP_Constraint_SetAxisMotorMaxForce === 'function') HK.HP_Constraint_SetAxisMotorMaxForce(constraintId, axis, 1000);
  }
}

// Walk up the node hierarchy (including the node itself) to the nearest node that owns a body.
function resolveAncestorBody(nodeIndex) {
  let idx = nodeIndex;
  while (idx !== undefined) {
...
// Create a constraint between two bodies, anchored at the joint node's world frame.
function setupJoint(jointWorldMat, parentBodyId, childBodyId, jointDef, enableCollision) {
  if (typeof HK.HP_Constraint_Create !== 'function') {
    console.warn('[JointTypes] HP_Constraint_Create is not available in this Havok build');
    return false;
  }
  ...
  const ok = enumToNumber(HK.Result.RESULT_OK);
  if (!Number.isNaN(rc) && !Number.isNaN(ok) && rc !== ok) {
    console.warn('[JointTypes] HP_Constraint_Create returned', created[0]);
    return false;
  }
  const constraintId = created[1];

  HK.HP_Constraint_SetParentBody(constraintId, parentBodyId);
  HK.HP_Constraint_SetChildBody(constraintId, childBodyId);

  const jdec = mat4Decompose(jointWorldMat);
  const jPos = jdec.t;
  const jQuat = jdec.r;

  // Express the joint frame (pivot + main/perp axes) in each body's local space.
  const [, pPos] = HK.HP_Body_GetPosition(parentBodyId);
  const [, pOri] = HK.HP_Body_GetOrientation(parentBodyId);
  const pInv = quatInvert([pOri[0], pOri[1], pOri[2], pOri[3]]);
  HK.HP_Constraint_SetAnchorInParent(constraintId,
    quatMulVec3(pInv, vecSub(jPos, [pPos[0], pPos[1], pPos[2]])),
    quatMulVec3(pInv, quatMulVec3(jQuat, [1, 0, 0])),
    quatMulVec3(pInv, quatMulVec3(jQuat, [0, 1, 0]))
  );

  const [, cPos] = HK.HP_Body_GetPosition(childBodyId);
  const [, cOri] = HK.HP_Body_GetOrientation(childBodyId);
  const cInv = quatInvert([cOri[0], cOri[1], cOri[2], cOri[3]]);
  HK.HP_Constraint_SetAnchorInChild(constraintId,
    quatMulVec3(cInv, vecSub(jPos, [cPos[0], cPos[1], cPos[2]])),
    quatMulVec3(cInv, quatMulVec3(jQuat, [1, 0, 0])),
    quatMulVec3(cInv, quatMulVec3(jQuat, [0, 1, 0]))
  );

  configureConstraintAxes(constraintId, jointDef);

  if (typeof HK.HP_Constraint_SetCollisionsEnabled === 'function') HK.HP_Constraint_SetCollisionsEnabled(constraintId, enableCollision === true);
  if (typeof HK.HP_Constraint_SetEnabled === 'function') HK.HP_Constraint_SetEnabled(constraintId, true);
  return true;
}
...
function createDebugEntity(shapeResult, color) {
  ...
  if (type === 'mesh' && shapeResult.positions && shapeResult.scaledIndices) {
    ...
    try {
      ...
      console.warn('[JointTypes] mesh wireframe failed, using box:', e);
  ...
  try { entity.getMesh().calcBaryCentricCoord(); } catch (e) { console.warn('[JointTypes] calcBaryCentricCoord failed:', e); }
...
const load = async function () {
  ...
  // (climbing parents so group nodes are covered too).
  ...
  for (const entity of modelRenderPass.entities) {
    ...
    while (cur) {
      ...
      // Rhodonite appends a "_(NN)" suffix to disambiguate imported names; also index by the bare
      // name so glTF node names (Cube.001, ...) still resolve to their entity.
      if (nm) {
        const bare = nm.replace(/_\(\d+\)$/, '');
        if (bare !== nm && !nameToEntity.has(bare)) nameToEntity.set(bare, cur);
      }
      let parentSg = null;
      try {
  ...
  // child node index -> parent node index, for resolving joint connections by climbing ancestors.
  for (let i = 0; i < (gltfJson.nodes || []).length; i++) {
    for (const c of (gltfJson.nodes[i].children || [])) parentOf.set(c, i);
  }

  // Render one frame first so world matrices (for the compound bounding box) are ready.
  ...
  function makeNode(entity, bodyId, isDynamic, pos, rot) {
    const node = {
      entity, bodyId, isDynamic,
      initialPosition: [pos[0], pos[1], pos[2]],
      initialRotation: [rot[0], rot[1], rot[2], rot[3]],
    };
  ...
  // collider nodes form a container shape).
  if (typeof HK.HP_Shape_CreateContainer === 'function') {
    for (const nodeIndex of sceneNodeIndices) {
      ...
      const containerId = containerRes[1];
      ...
      for (const ci of childIndices) {
        ...
        const cShape = buildShape(cgeom, childDec.s, false);
        if (!cShape) continue;
        const rel = mat4Decompose(mat4Multiply(parentInv, childWorld));
        HK.HP_Shape_AddChild(containerId, cShape.shapeId, [
          [rel.t[0], rel.t[1], rel.t[2]],
          [rel.r[0], rel.r[1], rel.r[2], rel.r[3]],
          [1, 1, 1],
        ]);
      ...
      const motionType = parentMotion.isKinematic ? HK.MotionType.KINEMATIC : HK.MotionType.DYNAMIC;
      const bodyId = createBody(containerId, motionType, parentDec.t, parentDec.r, parentMotion);
      nodeIndexToBodyId.set(nodeIndex, bodyId);
      let entity = nodeIndexToEntity.get(nodeIndex) || (nd.name ? nameToEntity.get(nd.name) : null);
      const node = makeNode(entity, bodyId, true, parentDec.t, parentDec.r);
      node.initialLinearVelocity = Array.isArray(parentMotion.linearVelocity) ? parentMotion.linearVelocity.slice() : [0, 0, 0];
      node.initialAngularVelocity = Array.isArray(parentMotion.angularVelocity) ? parentMotion.angularVelocity.slice() : [0, 0, 0];
  ...
  for (const nodeIndex of sceneNodeIndices) {
    ...
    // Prefer name-based mapping: gltfNodeIndex is unreliable in this Rhodonite build, and the
    // JointTypes nodes have unique names (Cube, Cube.001, ...), so the name is the safer key.
    let entity = (nodeDef.name && nameToEntity.get(nodeDef.name)) || nodeIndexToEntity.get(nodeIndex);
    if (!entity) console.warn('[JointTypes] no entity for node', nodeIndex, JSON.stringify(nodeDef.name));
    ...
    const motionType = isDynamic
      ? (motion.isKinematic ? HK.MotionType.KINEMATIC : HK.MotionType.DYNAMIC)
      : HK.MotionType.STATIC;
    const bodyId = createBody(shapeResult.shapeId, motionType, bodyPosition, rotation, motion);
    nodeIndexToBodyId.set(nodeIndex, bodyId);
    const node = makeNode(entity, bodyId, isDynamic, bodyPosition, rotation);
    if (isDynamic) {
      node.initialLinearVelocity = Array.isArray(motion.linearVelocity) ? motion.linearVelocity.slice() : [0, 0, 0];
      node.initialAngularVelocity = Array.isArray(motion.angularVelocity) ? motion.angularVelocity.slice() : [0, 0, 0];
    }
  ...
  // Joint pass: connect bodies with constraints. The joint node ("jointSpaceA") is a child of the
  // body that owns the joint; connectedNode ("jointSpaceB") is a child of the connected body — so
  // both are resolved by climbing to the nearest ancestor that owns a body.
  let jointsFound = 0, jointsCreated = 0;
  for (let nodeIndex = 0; nodeIndex < (gltfJson.nodes || []).length; nodeIndex++) {
    const jointExt = gltfJson.nodes[nodeIndex]?.extensions?.KHR_physics_rigid_bodies?.joint;
    if (!jointExt) continue;
    jointsFound++;
    const jointDef = physicsJoints[jointExt.joint];
    if (!jointDef) continue;

    const parentBodyId = resolveAncestorBody(nodeIndex);
    const childBodyId = resolveAncestorBody(jointExt.connectedNode);
    if (parentBodyId === undefined || childBodyId === undefined) {
      console.warn('[JointTypes] joint node', nodeIndex, 'missing parent/child body (parent=', parentBodyId, 'connectedNode=', jointExt.connectedNode, ')');
      continue;
    }
    const jointWorld = worldMatrices.get(nodeIndex);
    if (!jointWorld) continue;

    try {
      if (setupJoint(jointWorld, parentBodyId, childBodyId, jointDef, jointExt.enableCollision)) jointsCreated++;
    } catch (e) {
      console.warn('[JointTypes] setupJoint failed for node', nodeIndex, e);
    }
  }
  if (jointsCreated < jointsFound) {
    console.warn('[JointTypes] only', jointsCreated, 'of', jointsFound, 'joints created');
  }
  ...
  setInterval(() => {
    ...
    for (const node of dynamicNodes) {
      ...
      if (p[1] < RESET_Y_THRESHOLD || p[1] > RESET_Y_THRESHOLD_TOP) {
        HK.HP_Body_SetPosition(node.bodyId, node.initialPosition);
        HK.HP_Body_SetOrientation(node.bodyId, node.initialRotation);
        HK.HP_Body_SetLinearVelocity(node.bodyId, node.initialLinearVelocity || [0, 0, 0]);
        HK.HP_Body_SetAngularVelocity(node.bodyId, node.initialAngularVelocity || [0, 0, 0]);
      }
    }
    ...
    for (const node of physicsNodes) {
      ...
      if (node.childDebug) {
        const parentMat = mat4FromTRS([pos[0], pos[1], pos[2]], [ori[0], ori[1], ori[2], ori[3]], [1, 1, 1]);
