...
// --- Collision filtering (KHR_physics_rigid_bodies collisionFilters) ---
// Each filter lists the systems it belongs to (collisionSystems) and the systems it collides
// with (collideWithSystems). Map every system name to a unique bit, then pack membership /
// collide-with masks into Havok's HP_Shape_SetFilterInfo [membership, collideWith].
...
function createDebugEntity(shapeResult, color) {
  ...
  if (type === 'mesh' && shapeResult.positions && shapeResult.scaledIndices) {
    ...
    try {
      ...
      console.warn('[Filtering] mesh wireframe failed, using box:', e);
  ...
  try { entity.getMesh().calcBaryCentricCoord(); } catch (e) { console.warn('[Filtering] calcBaryCentricCoord failed:', e); }
...
const load = async function () {
  ...
  for (const nodeIndex of sceneNodeIndices) {
    ...
    if (!entity) console.warn('[Filtering] no entity for node', nodeIndex, nodeDef.name);
    ...
    // Collision filter: which systems this collider belongs to / collides with.
    const filterIndex = physicsExt.collider.collisionFilter;
    if (filterIndex !== undefined && filterDefs[filterIndex]) {
      applyCollisionFilter(shapeResult.shapeId, filterDefs[filterIndex], systemBitMap);
    }
