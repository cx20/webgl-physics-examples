...
function checkResult(result, label) {
  ...
  console.warn('[Restitution] ' + label + ' returned:', result);
}
...
function createDebugEntity(shapeResult, color) {
  ...
  try { entity.getMesh().calcBaryCentricCoord(); } catch (e) { console.warn('[Restitution] calcBaryCentricCoord failed:', e); }
...
const load = async function () {
  ...
  for (const nodeIndex of sceneNodeIndices) {
    ...
    if (!entity) console.warn('[Restitution] no entity for node', nodeIndex, nodeDef.name);
