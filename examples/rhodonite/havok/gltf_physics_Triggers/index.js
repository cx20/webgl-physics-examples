...
const triggerEntries = []; // sensor volumes: { bodyId, debugEntity, mat, worldAABB, active }
...
const DEBUG_COLOR_TRIGGER = [0.2, 0.8, 1.0, 1.0];          // sensor volume (idle)
const DEBUG_COLOR_TRIGGER_ACTIVE = [1.0, 0.85, 0.1, 1.0];  // sensor volume (a body is inside)
...
function createDebugEntity(shapeResult, color) {
  ...
  if (type === 'mesh' && shapeResult.positions && shapeResult.scaledIndices) {
    ...
    try {
      ...
      console.warn('[Triggers] mesh wireframe failed, using box:', e);
  ...
  try { entity.getMesh().calcBaryCentricCoord(); } catch (e) { console.warn('[Triggers] calcBaryCentricCoord failed:', e); }
  entity.debugMaterial = mat;  // kept so triggers can recolor their wireframe on overlap
  debugEntities.push(entity);
  return entity;
}

// Axis-aligned overlap test between each sensor volume and the dynamic body centers; recolor
// the trigger wireframe when something is inside. (Mirrors the three.js sample's AABB test.)
function updateTriggers() {
  for (const entry of triggerEntries) {
    if (!entry.worldAABB) entry.worldAABB = getEntityAABB(entry.debugEntity);
    const box = entry.worldAABB;
    let triggered = false;
    if (box) {
      const hx = box.size[0] / 2, hy = box.size[1] / 2, hz = box.size[2] / 2;
      for (const dyn of dynamicNodes) {
        const [, p] = HK.HP_Body_GetPosition(dyn.bodyId);
        if (Math.abs(p[0] - box.center[0]) <= hx &&
            Math.abs(p[1] - box.center[1]) <= hy &&
            Math.abs(p[2] - box.center[2]) <= hz) {
          triggered = true;
          break;
        }
      }
    }
    if (entry.active !== triggered) {
      entry.active = triggered;
      if (entry.mat) {
        try {
          entry.mat.setParameter('baseColorFactor',
            Rn.Vector4.fromCopyArray4(triggered ? DEBUG_COLOR_TRIGGER_ACTIVE : DEBUG_COLOR_TRIGGER));
        } catch (e) {}
      }
    }
  }
...
const load = async function () {
  ...
  for (const entity of modelRenderPass.entities) {
    ...
    while (cur) {
      ...
      // name so glTF node names still resolve to their entity.
      if (nm) {
  ...
  for (const nodeIndex of sceneNodeIndices) {
    ...
    if (!entity) console.warn('[Triggers] no entity for node', nodeIndex, nodeDef.name);
  ...
  // Trigger pass: sensor volumes (KHR_physics_rigid_bodies.trigger). These can be nested under
  // non-physics group nodes, so iterate every glTF node and place them by world matrix. The shape
  // is flagged as a trigger (no collision response); overlap is detected manually for the highlight.
  for (let nodeIndex = 0; nodeIndex < (gltfJson.nodes || []).length; nodeIndex++) {
    if (processed.has(nodeIndex)) continue;
    const nodeDef = gltfJson.nodes[nodeIndex];
    const tgeom = nodeDef?.extensions?.KHR_physics_rigid_bodies?.trigger?.geometry;
    if (!tgeom) continue;
    const world = worldMatrices.get(nodeIndex);
    if (!world) continue;
    const dec = mat4Decompose(world);

    const shapeResult = buildShape(tgeom, dec.s, false);
    if (!shapeResult) continue;
    if (typeof HK.HP_Shape_SetTrigger === 'function') {
      try { HK.HP_Shape_SetTrigger(shapeResult.shapeId, true); } catch (e) {}
    }
    createBody(shapeResult.shapeId, HK.MotionType.STATIC, dec.t, dec.r, null);

    const debugEntity = createDebugEntity(shapeResult, DEBUG_COLOR_TRIGGER);
    debugEntity.getTransform().localPosition = Rn.Vector3.fromCopyArray(dec.t);
    debugEntity.getTransform().localRotation = Rn.Quaternion.fromCopyArray(dec.r);
    triggerEntries.push({ debugEntity, mat: debugEntity.debugMaterial, worldAABB: null, active: null });
    processed.add(nodeIndex);
  }
