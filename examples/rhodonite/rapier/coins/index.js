...
// Rhodonite (rendering) + Rapier (physics, raw world/rigid-body API). Coins (cylinder visuals with
// sphere colliders) are spawned at the Duck model's vertices and rain into a pen. Click to launch
// them upward. Mirrors the Rhodonite + Havok coins sample.
...
// Per-type collider data. The visual is a flat cylinder coin, but the collider is a sphere
// (ball) of radius diameter/2 - matching the Havok sample.
...
function spawnCoins(coinPositions, meshesByType, debugSphereMesh, typeData) {
  for (let i = 0; i < coinPositions.length; i++) {
    ...
    const p = coinPositions[i];

    const body = world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(p[0], p[1], p[2]));
    world.createCollider(RAPIER.ColliderDesc.ball(td.radius).setDensity(1), body);
    ...
    coins.push({ body, coinEntity, debugEntity });
  }
}
...
async function load() {
  ...
  createStaticBox([20, 1, 20], [0, GROUND_Y - 0.5, 0]);
  ...
  window.addEventListener('click', () => {
    for (const coin of coins) {
      coin.body.setLinvel({ x: rand(-1, 1), y: rand(3, 6), z: rand(-1, 1) }, true);
    }
  });
  ...
  const draw = function() {
    ...
    for (let i = 0; i < coins.length; i++) {
      ...
      const ori = coin.body.rotation();

      coin.coinEntity.getTransform().localPosition = Rn.Vector3.fromCopyArray([pos.x, pos.y, pos.z]);
      coin.coinEntity.getTransform().localRotation = Rn.Quaternion.fromCopyArray([ori.x, ori.y, ori.z, ori.w]);

      coin.debugEntity.getTransform().localPosition = Rn.Vector3.fromCopyArray([pos.x, pos.y, pos.z]);
      coin.debugEntity.getTransform().localRotation = Rn.Quaternion.fromCopyArray([ori.x, ori.y, ori.z, ori.w]);

      if (pos.y < -50) {
        const newPos = getNextPosition();
        coin.body.setTranslation({ x: newPos[0], y: newPos[1], z: newPos[2] }, true);
        coin.body.setRotation({ x: 0, y: 0, z: 0, w: 1 }, true);
