import {
    ...
    setPhysicsBodyShape, setPhysicsShapeFilterCollideMask, setPhysicsShapeFilterMembershipMask,
...
// Babylon Lite has no built-in glTF-physics loader, so we parse the glb's physics extensions
...
// Babylon.js (full) example. This example demonstrates KHR collision filtering: blue and green bodies
// belong to different collision systems and pass through each other while colliding with the ground.
// It also has a dynamic compound body (a container shape with a blue and a green convex-hull child,
// each with its own filter, plus decorative wires).
...
async function main() {
    ...
    // Map each named collision system to a bit, then turn a collisionFilter index into Havok
    // membership/collide masks. Two shapes collide only if each one's membership intersects the
    // other's collide mask.
    const systemBit = {};
    let nextBit = 1;
    for (const f of filterDefs) {
        for (const sys of [...(f.collisionSystems || []), ...(f.collideWithSystems || [])]) {
            if (!(sys in systemBit)) { systemBit[sys] = nextBit; nextBit <<= 1; }
        }
    }
    const masksFor = (filterIndex) => {
        const f = filterIndex !== undefined ? filterDefs[filterIndex] : null;
        if (!f) return { membership: 0xffff, collide: 0xffff };
        let membership = 0;
        let collide = 0;
        for (const s of (f.collisionSystems || [])) membership |= systemBit[s];
        for (const s of (f.collideWithSystems || [])) collide |= systemBit[s];
        return { membership: membership || 0xffff, collide: collide || 0xffff };
    };
    ...
    const setMaterial = (shape, matDef) => {
    ...
    const applyFilter = (shape, filterIndex) => {
        const masks = masksFor(filterIndex);
        setPhysicsShapeFilterMembershipMask(world, shape, masks.membership);
        setPhysicsShapeFilterCollideMask(world, shape, masks.collide);
    };

    // A "compound" body has motion but no collider of its own; its descendant collider nodes provide
    // the shapes. Collect those child colliders so the main pass skips them.
    const consumed = new Set();
    const compounds = [];
    const collectChildColliders = (nodeIndex, out) => {
        for (const childIndex of (json.nodes[nodeIndex].children || [])) {
            const childPhysics = json.nodes[childIndex].extensions?.KHR_physics_rigid_bodies;
            if (childPhysics?.collider?.geometry) { out.push(childIndex); consumed.add(childIndex); }
            collectChildColliders(childIndex, out);
        }
    };
    for (let i = 0; i < json.nodes.length; i++) {
        ...
        if (physics?.motion && !physics.collider && !physics.trigger && (json.nodes[i].children || []).length) {
            ...
            collectChildColliders(i, childColliders);
            if (childColliders.length) compounds.push({ index: i, childColliders });
        }
    }
    const compoundByIndex = new Map(compounds.map((c) => [c.index, c]));

    // Reparent a node's loaded subtree under an anchor and return the anchor's LH transform.
    ...
    const buildChildShape = (childIndex) => {
        const geom = json.nodes[childIndex].extensions.KHR_physics_rigid_bodies.collider.geometry;
        if (geom.shape !== undefined) {
            // Primitive child: base size from the shape def; the parent-relative transform (added
            // below) carries the world scale.
            return createPhysicsShape(world, primitiveShapeOptions(shapeDefs[geom.shape], [1, 1, 1]));
        }
        const childTN = nodeToTN.get(childIndex);
        const type = geom.convexHull ? PhysicsShapeType.CONVEX_HULL : PhysicsShapeType.MESH;
        return createPhysicsShape(world, { type, mesh: childTN, includeChildMeshes: true });
    };
    ...
    for (let nodeIndex = 0; nodeIndex < json.nodes.length; nodeIndex++) {
        ...
        // Compound body: a container of the descendant child shapes (each with its own filter).
        ...
        if (compound) {
            ...
            for (const childIndex of compound.childColliders) {
                const childShape = buildChildShape(childIndex);
                applyFilter(childShape, json.nodes[childIndex].extensions.KHR_physics_rigid_bodies.collider.collisionFilter);
                setMaterial(childShape, materialDefs[json.nodes[childIndex].extensions.KHR_physics_rigid_bodies.collider.physicsMaterial]);
        ...
        applyFilter(shape, physics.collider.collisionFilter);
        setMaterial(shape, materialDefs[physics.collider.physicsMaterial]);
