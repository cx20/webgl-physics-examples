...
function initPhysics() {
    ...
    // Load the carrot texture via the asset system and re-update the material once it
    // arrives, so the diffuse map is reliably picked up by the cone material's shader.
    carrotMat = new pc.StandardMaterial();
    carrotMat.update();
    app.assets.loadFromUrl(CARROT_TEX, 'texture', (err, asset) => {
        if (err) { console.error('Failed to load carrot texture:', err); return; }
        carrotMat.diffuseMap = asset.resource;
        carrotMat.update();
    });
