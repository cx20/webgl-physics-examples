// Filament + Havok — glTF Physics "Triggers" sample.
//
// Renders the Khronos glTF_Physics "Triggers" model with Google Filament and simulates it with
// Havok via the KHR_physics_rigid_bodies / KHR_implicit_shapes extensions. This scene shows
// trigger volumes (KHR_physics_rigid_bodies.trigger): static sensors that detect overlap without
// blocking. A cube falls through the (translucent green) trigger volumes; each trigger's
// wireframe turns red while a dynamic body is inside it.
//
...
const DEBUG_COLOR_TRIGGER_IDLE = [0.2, 0.8, 1.0, 1.0];   // cyan   = trigger, nothing inside
const DEBUG_COLOR_TRIGGER_ACTIVE = [1.0, 0.2, 0.2, 1.0]; // red    = trigger, body inside
...
// Physics <-> Filament bookkeeping; wireframeEntity / wireframeIdle / wireframeActive are filled in
// by loadWireframeAsset().
...
const triggerEntries = []; // trigger volumes: { shape, initPos, initRot, worldAABB, active, lastActive, wireframeIdle, wireframeActive }
...
// World-space AABB of a shape placed at (pos, rot). The shape's local AABB / size is already in
// scaled coords, so only rotation + translation are applied.
function computeWorldAABB(shape, pos, rot) {
  let mn, mx;
  if (shape.aabb) { mn = shape.aabb.min; mx = shape.aabb.max; }
  else { const h = [shape.size[0] / 2, shape.size[1] / 2, shape.size[2] / 2]; mn = [-h[0], -h[1], -h[2]]; mx = [h[0], h[1], h[2]]; }
  const q = quat.fromValues(rot[0], rot[1], rot[2], rot[3]);
  const min = [Infinity, Infinity, Infinity], max = [-Infinity, -Infinity, -Infinity];
  for (let cx = 0; cx < 2; cx++) for (let cy = 0; cy < 2; cy++) for (let cz = 0; cz < 2; cz++) {
    const corner = vec3.fromValues(cx ? mx[0] : mn[0], cy ? mx[1] : mn[1], cz ? mx[2] : mn[2]);
    vec3.transformQuat(corner, corner, q);
    for (let k = 0; k < 3; k++) { const w = corner[k] + pos[k]; min[k] = Math.min(min[k], w); max[k] = Math.max(max[k], w); }
  }
  return { min, max };
}
...
async function initPhysicsFromUrl(meshUrl, filamentAsset, filamentEngine) {
  ...
  // Triggers: static sensor volumes. We do NOT add a Havok body (so dynamic bodies pass straight
  // through); overlap is detected manually (AABB vs dynamic body centers) to drive the highlight.
  for (let i = 0; i < nodes.length; i++) {
    const physExt = nodes[i].extensions?.KHR_physics_rigid_bodies;
    const tgeom = physExt?.trigger?.geometry;
    if (!tgeom) continue;
    const ws = vec3.create();
    mat4.getScaling(ws, worldMats[i]);
    let shape;
    if (tgeom.shape !== undefined) {
      const sd = shapeDefs[tgeom.shape];
      if (!sd) continue;
      shape = createImplicitShape(sd, ws, null, null);
    } else {
      shape = createMeshShape(gltfJson, accessors, worldMats[i], tgeom, null, null);
    }
    if (!shape) continue;
    const wPos = vec3.create(); mat4.getTranslation(wPos, worldMats[i]);
    const wRot = quat.create(); getRotationFromMat(wRot, worldMats[i]);
    const initPos = [wPos[0], wPos[1], wPos[2]];
    const initRot = [wRot[0], wRot[1], wRot[2], wRot[3]];
    triggerEntries.push({ shape, initPos, initRot, worldAABB: computeWorldAABB(shape, initPos, initRot), active: false });
    expandSceneBounds(initPos, boundRadiusFor(shape));
  }

  console.log('[Filament+Havok] physics ready:', physicsNodes.length, 'dynamic,', staticBodies.length, 'static,', triggerEntries.length, 'triggers');
}

// Mark each trigger active when any dynamic body's center is inside its world AABB.
function updateTriggers() {
  for (const t of triggerEntries) {
    let active = false;
    for (const dyn of physicsNodes) {
      const p = HK.HP_Body_GetPosition(dyn.bodyId)[1];
      const a = t.worldAABB;
      if (p[0] >= a.min[0] && p[0] <= a.max[0] && p[1] >= a.min[1] && p[1] <= a.max[1] && p[2] >= a.min[2] && p[2] <= a.max[2]) { active = true; break; }
    }
    t.active = active;
  }
}
...
function physicsStep() {
  ...
  // Trigger active/idle state changes -> swap which wireframe entity is in the scene.
  if (showWireframe && scene) {
    for (const t of triggerEntries) {
      if (t.active === t.lastActive) continue;
      if (t.active) {
        if (t.wireframeIdle) scene.remove(t.wireframeIdle);
        if (t.wireframeActive) scene.addEntity(t.wireframeActive);
      } else {
        if (t.wireframeActive) scene.remove(t.wireframeActive);
        if (t.wireframeIdle) scene.addEntity(t.wireframeIdle);
      }
...
async function loadWireframeAsset() {
  ...
  for (let i = 0; i < physicsNodes.length; i++) {
    const v = debugVertsFor(physicsNodes[i].shape);
  ...
  for (let i = 0; i < staticBodies.length; i++) {
    const v = debugVertsFor(staticBodies[i].shape);
  ...
  // Each trigger gets two entities — one with the idle (cyan) colour, one with active (red).
  // Only one is in the scene at a time, swapped in physicsStep based on t.active.
  for (let i = 0; i < triggerEntries.length; i++) {
    const v = debugVertsFor(triggerEntries[i].shape);
    meshes.push({ positions: v, indices: seqIndices(v.length / 3), color: DEBUG_COLOR_TRIGGER_IDLE });
    nodes.push({ name: 'triggerIdleWire' + i, meshIndex: meshes.length - 1 });
    meshes.push({ positions: v, indices: seqIndices(v.length / 3), color: DEBUG_COLOR_TRIGGER_ACTIVE });
    nodes.push({ name: 'triggerActiveWire' + i, meshIndex: meshes.length - 1 });
  }
  ...
  for (let i = 0; i < triggerEntries.length; i++) {
    triggerEntries[i].wireframeIdle = a.getEntitiesByName('triggerIdleWire' + i)[0] || null;
    triggerEntries[i].wireframeActive = a.getEntitiesByName('triggerActiveWire' + i)[0] || null;
    triggerEntries[i].lastActive = false;
    // Start with the active variant removed; only the idle one is visible.
    if (triggerEntries[i].wireframeActive) scene.remove(triggerEntries[i].wireframeActive);
  }
  // Place static + trigger wireframes once at their initial transforms.
  const tcm = engine.getTransformManager();
  tcm.openLocalTransformTransaction();
  function setInitial(entity, p, r) {
    if (!entity) return;
  ...
  for (const sb of staticBodies) {
    ...
    setInitial(sb.wireframeEntity, pr[1], qr[1]);
  }
  for (const t of triggerEntries) {
    setInitial(t.wireframeIdle, t.initPos, t.initRot);
    setInitial(t.wireframeActive, t.initPos, t.initRot);
  }
  tcm.commitLocalTransformTransaction();
  return a;
}
...
function setWireframeVisible(visible) {
  ...
  for (const t of triggerEntries) {
    if (visible) {
      // Re-add the variant matching the current active state.
      if (t.active && t.wireframeActive) scene.addEntity(t.wireframeActive);
      else if (!t.active && t.wireframeIdle) scene.addEntity(t.wireframeIdle);
    } else {
      if (t.wireframeIdle) scene.remove(t.wireframeIdle);
      if (t.wireframeActive) scene.remove(t.wireframeActive);
    }
...
async function main() {
  ...
  // Load the model — pop renderables only so the model's KHR_lights_punctual lights stay out (they
  // trip "glDrawElementsInstanced: uniform buffer too small" at feature level 1). Lighting is the
  // IBL + a directional SUN.
