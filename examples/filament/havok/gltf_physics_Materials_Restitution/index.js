// Filament + Havok — glTF Physics "Materials_Restitution" sample.
//
// Renders the Khronos glTF_Physics "Materials_Restitution" model with Google Filament and
// simulates it with Havok, driven by the KHR_physics_rigid_bodies / KHR_implicit_shapes
// extensions embedded in the GLB (box floor + sphere colliders with per-collider restitution
// materials — balls bounce to heights set by their restitution / bounciness).
//
...
const physicsNodes = [];   // dynamic bodies: { entity, bodyId, nodeScale, ..., shapeType, size, radius, wireframeEntity }
const staticBodies = [];   // static bodies:  { bodyId, shapeType, size, radius, wireframeEntity }
...
function applyPhysicsMaterial(shapeId, materialDef) {
  ...
  // Order: [dynamicFriction, staticFriction, restitution, frictionCombine, restitutionCombine].
  // Restitution uses MAXIMUM combine so a bouncy ball keeps bouncing on a non-bouncy floor.
...
// Build a Havok shape for a KHR_implicit_shapes def. Materials_Restitution uses a box floor and
// spheres, so only those two are handled. Returns { shapeId, shapeType, size, radius, boundRadius }.
function createShape(shapeDef, worldScale, motionDef, materialDef) {
  let shapeId, shapeType, size, radius = 0, boundRadius;
  if (shapeDef.box) {
    ...
    if (motionDef) {
      ...
      checkResult(HK.HP_Shape_SetDensity(shapeId, motionDef.mass !== undefined ? motionDef.mass / volume : 1), 'HP_Shape_SetDensity');
    }
    boundRadius = 0.5 * Math.hypot(size[0], size[1], size[2]);
  } else if (shapeDef.sphere) {
    const avgScale = (Math.abs(worldScale[0]) + Math.abs(worldScale[1]) + Math.abs(worldScale[2])) / 3;
    radius = (shapeDef.sphere.radius || 0.5) * avgScale;
    shapeType = 'sphere';
    ...
    if (motionDef && motionDef.mass !== undefined && motionDef.mass > 0) {
      const volume = Math.max((4 / 3) * Math.PI * radius * radius * radius, 0.0001);
  ...
  return { shapeId, shapeType, size, radius, boundRadius };
}
...
// Parse the GLB's physics extensions and build the Havok world. Materials_Restitution uses
// KHR_implicit_shapes box (floor) + sphere (balls) colliders, static + dynamic.
async function initPhysicsFromUrl(meshUrl, filamentAsset, filamentEngine) {
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    const shape = createShape(shapeDef, worldScale, motionDef, matDef);
    if (!shape) continue; // unsupported shape type
    ...
    expandSceneBounds(initPos, shape.boundRadius);
    ...
    if (motionDef) {
      ...
      const entry = { entity, bodyId, nodeScale, initPos, initRot, parentInvWorldMat, shapeType: shape.shapeType, size: shape.size, radius: shape.radius };
      ...
      staticBodies.push({ bodyId, shapeType: shape.shapeType, size: shape.size, radius: shape.radius });
    }
  }
...
function makeSphereLineVerts(radius, segments = 16) {
  const v = [];
  for (let c = 0; c < 3; c++) {           // three great circles (XY, XZ, YZ)
...
function shapeVertsFor(entry) {
  return entry.shapeType === 'sphere'
    ? makeSphereLineVerts(entry.radius)
    : makeBoxLineVerts(entry.size[0], entry.size[1], entry.size[2]);
}
...
async function loadWireframeAsset() {
  ...
  for (let i = 0; i < physicsNodes.length; i++) {
    const v = shapeVertsFor(physicsNodes[i]);
  ...
  for (let i = 0; i < staticBodies.length; i++) {
    const v = shapeVertsFor(staticBodies[i]);
...
async function main() {
  ...
  // Load the model. addEntities() would include the KHR_lights_punctual lights which trips
  // "glDrawElementsInstanced: uniform buffer too small" on base_lit_opaque at feature level 1, so
  // we use popRenderable() to add only renderables. Lighting comes from IBL + a directional SUN.
