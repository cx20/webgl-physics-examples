...
function initPhysics() {
    ...
    // Pass 2: Process trigger geometry nodes
    for (let nodeIndex = 0; nodeIndex < modelAsset.nodes.length; nodeIndex++) {
        const node = modelAsset.nodes[nodeIndex];
        if (!node.physicsExt || !node.physicsExt.trigger || !node.physicsExt.trigger.geometry) {
            continue;
        }

        const geomDef = node.physicsExt.trigger.geometry;
        const shapeIndex = geomDef.shape;

        let shapeResult;
        if (shapeIndex === undefined) {
            shapeResult = createMeshPhysicsShape(node, geomDef, null, null);
        } else {
            const shapeDef = shapeDefs[shapeIndex];
            shapeResult = createPhysicsShape(node, shapeDef, null, null);
        }
        ...
        if (typeof HK.HP_Shape_SetTrigger === 'function') {
            HK.HP_Shape_SetTrigger(shapeId, true);
        }
        ...
        node.bodyId = createBody(shapeId, HK.MotionType.STATIC, node.initialPosition, node.initialRotation, false, null, undefined);
        physicsNodes.push(node);
    }
}
