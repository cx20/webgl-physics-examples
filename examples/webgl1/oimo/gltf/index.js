...
function initPhysics(collisionSize) {
    ...
    // Give the model a visible spin at spawn, similar to existing glTF reference.
    duckBody.angularVelocity.set(0, 0, 3.5);
}

function updateDuckWorldMatrix() {
    const p = duckBody.getPosition();
    const q = duckBody.getQuaternion();

    const bodyRot = quat.fromValues(q.x, q.y, q.z, q.w);
    const bodyPos = vec3.fromValues(p.x, p.y, p.z);
...
function drawPhysicsDebugBox() {
    ...
    const p = duckBody.getPosition();
    const q = duckBody.getQuaternion();
    const rot = quat.fromValues(q.x, q.y, q.z, q.w);
    const model = mat4.create();
    mat4.fromRotationTranslation(model, rot, [p.x, p.y, p.z]);
...
async function main() {
    ...
    // Align visual mesh centroid to the Oimo box centroid for 1:1 debug fit.
