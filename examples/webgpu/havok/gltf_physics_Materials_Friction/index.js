...
function applyPhysicsMaterial(shapeId, materialDef) {
    ...
    HK.HP_Shape_SetMaterial(
        shapeId,
        [dynamicFriction, staticFriction, restitution, HK.MaterialCombine.MAXIMUM, HK.MaterialCombine.MINIMUM]
    );
}
...
async function main() {
    ...
    vec3.set(cameraCenter, centerX, centerY + Math.max(sizeY * 0.2, 0.8), centerZ);
    const diagonal = Math.sqrt(sizeX * sizeX + sizeY * sizeY + sizeZ * sizeZ);
    cameraRadius = Math.max(diagonal * 1.35, 12);
