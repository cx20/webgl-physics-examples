...
function initPhysics() {
    ...
    // First pass: compound bodies (motion, no collider)
    ...
    // Second pass: regular single-shape bodies
    for (let i = 0; i < modelAsset.nodes.length; i++) {
...
function render(timeMs) {
    ...
    if (SHOW_DEBUG_COLLIDERS) {
        ...
        for (const node of physicsNodes) {
            const color = node.physicsExt.motion ? [1.0, 0.35, 0.2, 1.0] : [0.0, 1.0, 0.0, 1.0];
            ...
            if (node.debugGPUMesh && node.debugTransformMatrix) {
                ...
                writeLineUniforms(debugLineUniformBuffer, debugModel, color);
