...
function createMeshPhysicsShape(node, colliderGeom, motionDef, materialDef) {
    ...
    // Allocate WASM heap memory, copy data, pass the offset, then free.
...
// Collect descendant nodes that have a collider but no motion (compound children).
// Stops recursion into nodes that are their own dynamic bodies (have motion).
function collectDescendantColliders(nodeIndex, result, excludedSet) {
    const node = modelAsset.nodes[nodeIndex];
    for (const childIdx of node.children) {
        const child = modelAsset.nodes[childIdx];
        if (child.physicsExt && child.physicsExt.motion) {
            continue; // separate dynamic body, not part of this compound
        }
        if (child.physicsExt && child.physicsExt.collider && child.physicsExt.collider.geometry) {
            result.push(childIdx);
            excludedSet.add(childIdx);
        }
        collectDescendantColliders(childIdx, result, excludedSet);
    }
}
...
function initPhysics() {
    ...
    // First pass: compound bodies (nodes that have motion but no collider; their children provide the shapes).
    const excludedNodeIndices = new Set();
    for (let i = 0; i < modelAsset.nodes.length; i++) {
        const node = modelAsset.nodes[i];
        if (!node.physicsExt || !node.physicsExt.motion || node.physicsExt.collider) {
            continue; // not a compound parent
        }

        const childColliderIndices = [];
        collectDescendantColliders(i, childColliderIndices, excludedNodeIndices);
        if (childColliderIndices.length === 0) continue;

        if (typeof HK.HP_Shape_CreateContainer !== 'function') {
            console.warn('[WARN] HP_Shape_CreateContainer not available, skipping compound body:', node.name);
            continue;
        }
        ...
        mat4.getTranslation(parentPos, node.restWorldMatrix);
        mat4.getRotation(parentRot, node.restWorldMatrix);
        const parentRotInv = quat.invert(quat.create(), parentRot);

        for (const childIdx of childColliderIndices) {
            const childNode = modelAsset.nodes[childIdx];
            const childGeom = childNode.physicsExt.collider.geometry;
            const childShapeIndex = childGeom.shape;
            const childMaterialDef = childNode.physicsExt.collider.physicsMaterial !== undefined
                ? materialDefs[childNode.physicsExt.collider.physicsMaterial]
                : null;

            let childShapeResult;
            if (childShapeIndex === undefined) {
                childShapeResult = createMeshPhysicsShape(childNode, childGeom, null, childMaterialDef);
            } else {
                childShapeResult = createPhysicsShape(childNode, shapeDefs[childShapeIndex], null, childMaterialDef);
            }
            if (!childShapeResult) continue;

            const childPos = vec3.create();
            const childRot = quat.create();
            mat4.getTranslation(childPos, childNode.restWorldMatrix);
            mat4.getRotation(childRot, childNode.restWorldMatrix);

            // Compute local transform relative to compound parent (rotation-only inverse, no scale)
        ...
        const position = [parentPos[0], parentPos[1], parentPos[2]];
        const rotation = [parentRot[0], parentRot[1], parentRot[2], parentRot[3]];
        node.initialPosition = position;
        node.initialRotation = rotation;
        node.debugSize = [1, 1, 1];

        const motionDef = node.physicsExt.motion;
        const motionType = motionDef.isKinematic ? HK.MotionType.KINEMATIC : HK.MotionType.DYNAMIC;
        const gravityFactor = motionDef.gravityFactor !== undefined ? motionDef.gravityFactor : undefined;
        node.bodyId = createBody(containerShapeId, motionType, position, rotation, true, motionDef, gravityFactor);
        physicsNodes.push(node);
        dynamicNodes.push(node);
    }

    // Second pass: regular single-shape bodies (skip compound children consumed above).
    for (let i = 0; i < modelAsset.nodes.length; i++) {
...
function buildPhysicsDebugMeshes() {
    if (typeof HK.HP_Shape_CreateDebugDisplayGeometry !== 'function') {
        console.warn('[DEBUG] HP_Shape_CreateDebugDisplayGeometry not available, falling back to box debug');
        return;
    }
    for (const node of physicsNodes) {
        const shapeRes = HK.HP_Body_GetShape(node.bodyId);
        if (!shapeRes) continue;
        const shapeHandle = shapeRes[1];

        const geomRes = HK.HP_Shape_CreateDebugDisplayGeometry(shapeHandle);
        if (!geomRes) continue;
        ...
        if (!infoRes) { HK.HP_DebugGeometry_Release(geomHandle); continue; }

        const info = infoRes[1]; // [posOffset, numVerts, triOffset, numTris]
        const posOffset = info[0];
        const numVerts  = info[1];
        const triOffset = info[2];
        const numTris   = info[3];

        // Copy data out of WASM heap before releasing
        ...
        // Convert each triangle (i0,i1,i2) into 3 line pairs for wireframe
        ...
        for (let t = 0; t < numTris; t++) {
            ...
            lineIndices[t * 6 + 0] = i0; lineIndices[t * 6 + 1] = i1;
            lineIndices[t * 6 + 2] = i1; lineIndices[t * 6 + 3] = i2;
            lineIndices[t * 6 + 4] = i2; lineIndices[t * 6 + 5] = i0;
        }
        ...
        gl.bufferData(gl.ELEMENT_ARRAY_BUFFER,
            useUint32 ? lineIndices : new Uint16Array(lineIndices),
            gl.STATIC_DRAW);
...
function updatePhysicsTransforms() {
    for (const node of physicsNodes) {
        ...
        mat4.fromRotationTranslationScale(node.worldMatrix, rot, pos, node.worldScale);

        // Debug transform: rotation + translation only (scale is already baked into shape geometry)
...
function drawPhysicsDebug() {
    ...
    for (const node of physicsNodes) {
        const isDynamic = !!node.physicsExt.motion;
        const color = isDynamic ? [1.0, 0.35, 0.2, 1.0] : [0.2, 0.9, 0.35, 1.0];

        if (node.debugGLMesh && node.debugTransformMatrix) {
            // Actual Havok shape wireframe
