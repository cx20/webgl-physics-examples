...
function createCubemapFromHDR(hdr, size = 256) {
    const faces = [
    ...
    for (const face of faces) {
        const faceData = new Float32Array(size * size * 3);
        let p = 0;
        ...
        gl.texImage2D(face, 0, gl.RGB16F, size, size, 0, gl.RGB, gl.FLOAT, faceData);
    }
...
function createMeshBuffers(positions, normals, uvs, indices) {
    ...
    return { vao, indexBuffer, indexCount, indexType, hasIndices: !!indices };
}
...
async function loadMarbleTemplates() {
    ...
    for (let m = 0; m < gltf.meshes.length; m++) {
        ...
        for (const primitive of meshDef.primitives) {
            ...
            primitives.push({
                mesh: createMeshBuffers(positions, normals, uvs, indices),
                texture,
                baseColor,
                ior,
                iridescenceFactor,
...
function drawMarbles() {
    ...
    for (const marble of marbles) {
        ...
        for (const prim of marble.primitives) {
            gl.bindVertexArray(prim.mesh.vao);
            gl.uniformMatrix4fv(uniforms.model, false, model);
