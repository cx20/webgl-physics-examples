...
const physicsNodes = [];
const dynamicNodes = [];
const triggerEntries = [];  // { object, worldBox, bodyId }
...
const tmpDynPos = new THREE.Vector3();
...
function createImplicitShape(worldScale, shapeDef, motionDef, materialDef) {
  ...
  if (shapeDef.type === 'box' && shapeDef.box) {
    ...
    const maxR = Math.max(rT * sXZ, rB * sXZ, 0.0001);
    if (typeof HK.HP_Shape_CreateCylinder === 'function') {
      ...
      const res = HK.HP_Shape_CreateBox([0, 0, 0], IDENTITY_QUATERNION, [maxR * 2, sHH * 2, maxR * 2]);
...
function makeTriggerMaterial(object) {
...
function updateTriggerHighlight(entry, active) {
  entry.object.traverse((child) => {
    if (!child.isMesh || !child.material) return;
    if (active) {
      child.material.emissive = new THREE.Color(0.4, 0.4, 0.0);
...
function checkTriggerOverlaps() {
  for (const entry of triggerEntries) {
    entry.worldBox.setFromObject(entry.object);
    let triggered = false;
    for (const dyn of dynamicNodes) {
      const pResult = HK.HP_Body_GetPosition(dyn.bodyId);
      if (pResult[0] !== HK.Result.RESULT_OK) continue;
      tmpDynPos.set(pResult[1][0], pResult[1][1], pResult[1][2]);
      if (entry.worldBox.containsPoint(tmpDynPos)) {
        triggered = true;
        break;
      }
    }
    updateTriggerHighlight(entry, triggered);
  }
}
...
async function loadModelAndBuildPhysics() {
  ...
  for (let i = 0; i < (json.nodes || []).length; i++) {
    ...
    // Trigger node
    if (physicsExt.trigger?.geometry) {
      const geomDef = physicsExt.trigger.geometry;
      let shapeResult;
      if (geomDef.shape !== undefined) {
        try {
          shapeResult = createImplicitShape(tmpWorldScale, shapeDefs[geomDef.shape], null, null);
        } catch (e) {
          console.warn('[Havok] Trigger createImplicitShape failed:', e.message);
          continue;
        }
      } else {
        shapeResult = createMeshShape(tmpWorldScale, geomDef, meshIndexToGeos, json.nodes, null);
        if (!shapeResult) continue;
      }

      // Mark shape as trigger (sensor)
      if (typeof HK.HP_Shape_SetTrigger === 'function') {
        HK.HP_Shape_SetTrigger(shapeResult.shapeId, true);
      }

      const bodyId = createBody(shapeResult.shapeId, HK.MotionType.STATIC, position, rotation, null);

      // Make the visual mesh semi-transparent for clear trigger visualization
      makeTriggerMaterial(object);
    ...
    // Regular collider node
    if (!physicsExt.collider?.geometry) continue;
    if (processedNodeIndices.has(i)) continue;
